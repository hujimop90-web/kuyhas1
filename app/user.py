...
def country_view(country_code):
    ...
    # Hitung total klaim user ini dalam 24 jam terakhir
    ...
    max_daily_claims = getattr(current_user, 'max_daily_claims', 5)
    if max_daily_claims is None:
        max_daily_claims = 5

    total_claims_left = getattr(current_user, 'total_claims_left', 20)
    if total_claims_left is None:
        total_claims_left = 20
    ...
    return render_template('user/country.html',
                           ...
                           total_claims_left=total_claims_left)
...
def api_country_data(country_code):
    ...
    # Verifikasi batas limit klaim harian (24 jam)
    from datetime import datetime, timedelta
    twenty_four_hours_ago = datetime.utcnow() - timedelta(hours=24)
    daily_claims_count = UserCookieClaim.query.filter(
        UserCookieClaim.user_id == current_user.id,
    ...
    # Query cookie_id yang sudah diklaim oleh 2 user atau lebih secara global
    exhausted_cookies_sq = sa_select(UserCookieClaim.cookie_id).group_by(
        UserCookieClaim.cookie_id
    ).having(
        func.count(UserCookieClaim.id) >= 2
    ).scalar_subquery()

    # Cari cookie yang belum di-claim oleh user ini DAN belum diklaim 2 user lain secara global
    ...
    try:
        ...
        # Kurangi kuota total klaim user
    ...
    # Hitung ulang total klaim harian setelah klaim baru ditambahkan
    new_daily_claims = UserCookieClaim.query.filter(
        UserCookieClaim.user_id == current_user.id,
        UserCookieClaim.claimed_at >= twenty_four_hours_ago
    ).count()
    ...
    return jsonify({
        ...
        'daily_claims_count': new_daily_claims,
        'max_daily_claims': limit
    })
...
def get_token(cookie_id):
    ...
    # JIKA GENERATE NFTOKEN GAGAL: Refund claim agar kuota user tidak hangus sia-sia
    if not result.get('success'):
        try:
            claim = UserCookieClaim.query.filter_by(
                user_id=current_user.id,
                cookie_id=cookie_id,
                service_type='netflix'
            ).order_by(UserCookieClaim.claimed_at.desc()).first()
            if claim:
                db.session.delete(claim)
                
                # Kembalikan kuota total klaim user
                if current_user.total_claims_left is not None:
                    current_user.total_claims_left += 1
                    
                db.session.commit()
                print(f"[QUOTA] Refunded claims for user {current_user.username} due to failed Netflix token generation.")
        except Exception as e:
            db.session.rollback()
            print(f"[QUOTA ERROR] Gagal melakukan refund: {e}")

    # Hitung ulang total klaim harian setelah klaim baru ditambahkan / di-refund
    from datetime import datetime, timedelta
    twenty_four_hours_ago = datetime.utcnow() - timedelta(hours=24)
    new_daily_claims = UserCookieClaim.query.filter(
        UserCookieClaim.user_id == current_user.id,
    ...
    result['daily_claims_count'] = new_daily_claims
    result['max_daily_claims'] = limit
